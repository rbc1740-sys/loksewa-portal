...
console.log(
  '[ok] geotechnical.json: added ' + added + ' explanations, remaining missing: ' + nowMissing
);
...
// founded 2 June 1936 AD ~ Jestha 1993 B.S.
{
  ...
  for (const { from, to } of pairs) {
    ...
    if (idx === -1)
      throw new Error('NOT FOUND in modern_history_of_nepal.json: ' + from.slice(0, 60));
    content = content.slice(0, idx) + to + content.slice(idx + from.length);
  }
  fs.writeFileSync(abs, content, 'utf8');
  console.log(
    '[ok] modern_history_of_nepal.json: fixed Prajaparishad option (1990 -> 1993 B.S.) and added explanation'
  );
}
