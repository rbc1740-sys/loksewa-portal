...
for (let i = 0; i < 40; i++) {
  if (await page.locator('#syllabus-categories button:has-text("Practice")').count() > 0) break;
...
const zenGone = await page.evaluate(() => ({
  ...
  zenModeText: !!document.querySelector('button:has-text("Zen")') || document.body.innerText.includes('Zen Mode'),
}));
console.log('1. ZEN REMOVED FROM DOM:', JSON.stringify(zenGone), JSON.stringify(zenGone) === '{"btn":false,"container":false,"flash":false,"zenModeText":false}' ? 'PASS' : 'CHECK');
...
const listState = await page.evaluate(() => ({
  listVisible: !!document.querySelector('#practice-list-view') && document.querySelector('#practice-list-view').offsetParent !== null,
