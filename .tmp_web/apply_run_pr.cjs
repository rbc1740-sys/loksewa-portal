...
function patch(file, splices, label) {
  ...
  for (const s of splices) {
    ...
    if (firstLine === undefined || !String(firstLine).includes(s.expectedFirst)) {
      console.error(`MISMATCH ${label} @${s.start}: expected contains ${JSON.stringify(s.expectedFirst)}, got ${JSON.stringify(firstLine)}`);
      process.exit(1);
    }
  }
...
// ------------------------- question-runner.tsx -------------------------
const runSplices = [];
const addRun = (start, end, body, expectedFirst) => runSplices.push({ start, end, lines: contentToLines(body), expectedFirst });

addRun(19, 19, `import { useSessionStore } from '../../src/engine/questionSession';
import { XP_PER_WRONG, correctRewardForTry } from '../../src/utils/gamification';`, 'useSessionStore');
addRun(39, 39, `  const answers = useSessionStore((s) => s.answers);
  const wrongAttempts = useSessionStore((s) => s.wrongAttempts);`, 'answers');
addRun(47, 47, `  const commitAnswer = useSessionStore((s) => s.commitAnswer);
  const recordWrongAttempt = useSessionStore((s) => s.recordWrongAttempt);`, 'commitAnswer');
addRun(58, 70, `  const selectedChoice = current ? answers[current.id] : undefined;
...
addRun(81, 89, `  function optionState(key: string): OptionState {
    const k = (key || '').toLowerCase();
    if (showFeedback) {
...
// ------------------------- practice-result.tsx -------------------------
const prSplices = [];
const addPr = (start, end, body, expectedFirst) => prSplices.push({ start, end, lines: contentToLines(body), expectedFirst });
addPr(25, 25, `  const answers = useSessionStore((s) => s.answers);
  const wrongAttempts = useSessionStore((s) => s.wrongAttempts);`, 'answers');
addPr(37, 40, `      const sel = answers[q.id];
      const triedWrong = (wrongAttempts[q.id] || []).length > 0;
      if (!sel && !triedWrong) skipped += 1;
      else if (sel) correct += 1;
      else incorrect += 1;`, 'sel');
addPr(53, 53, `  }, [questions, answers, timeSpent, wrongAttempts]);`, 'timeSpent');
addPr(55, 58, `  const wrongIds = useMemo(
    () =>
      questions
