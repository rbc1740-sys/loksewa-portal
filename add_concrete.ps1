...
$parts = @(
    "D:\gemini-code-1787397477528.json",
    "D:\gemini-code-1787397724381.json",
    "D:\gemini-code-1787398894056.json"
)
...
foreach ($part in $parts) {
    ...
    # Handle array format or wrapped-object format ({ "questions": [...] })
    $data = $null
    if ($parsed -is [System.Array]) {
        $data = $parsed
    } elseif ($parsed.questions) {
...
$outPath = Join-Path (Get-Location) "questions\concrete_technology.json"
$json = $out | ConvertTo-Json -Depth 8
[System.IO.File]::WriteAllText($outPath, $json, [System.Text.UTF8Encoding]::new($true))
Write-Output "Wrote $($out.Count) concrete technology questions to $outPath"
