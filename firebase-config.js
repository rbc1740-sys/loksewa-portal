...
(function initFirebase() {
  var isPlaceholder = Object.keys(firebaseConfig).some(function (key) {
    return (
      typeof firebaseConfig[key] === 'string' && firebaseConfig[key].indexOf('PASTE_YOUR_') === 0
    );
  });

  if (isPlaceholder) {
    console.warn(
      '[Loksewa Portal] Firebase config not filled in yet - running in local-only mode. Edit firebase-config.js to enable shared comments & voting.'
    );
    if (typeof setCloudStatus === 'function')
      setCloudStatus('local', 'Firebase config missing - running in local-only mode.');
    return;
  }
  ...
  if (typeof firebase === 'undefined') {
    ...
    if (typeof setCloudStatus === 'function')
      setCloudStatus('local', 'Firebase SDK failed to load (check internet/adblocker).');
    return;
  }
    ...
    function trySignIn() {
      ...
      firebase
        ...
        .then(function (cred) {
          ...
          if (typeof setCloudStatus === 'function')
            setCloudStatus('online', 'Connected to Firebase - shared comments & voting active.');
          document.dispatchEvent(new Event('firebase-ready'));
        })
        .catch(function (err) {
          console.warn(
            '[Loksewa Portal] Anonymous sign-in attempt ' + signInAttempts + ' failed:',
            err.code || '',
            err.message
          );
          if (typeof setCloudStatus === 'function')
            setCloudStatus('offline', 'Sign-in failed: ' + (err.code || err.message));
          if (signInAttempts < 5) {
            setTimeout(trySignIn, 2000 * signInAttempts);
          } else {
            console.error(
              '[Loksewa Portal] Could not connect after several attempts. Check that Authentication > Anonymous is enabled and Firestore rules are published.'
            );
          }
        });
    }
