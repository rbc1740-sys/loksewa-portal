$ErrorActionPreference = "Stop"

$sourceFile = "D:\gemini-code-1787399851522.json"
$targetFile = Join-Path (Get-Location) "questions\geotechnical.json"

function Clean-Text {
    param([string]$inText)
    if ([string]::IsNullOrEmpty($inText)) { return $inText }

    # Remove citation artifacts like [cite: 1] or [cite: 1, 3]
    $inText = [regex]::Replace($inText, '\[\s*cite\s*:\s*[\d,\s]+\s*\]', '')
    $inText = $inText -replace '\[\s*cite\s*\]', ''

    # LaTeX to readable text (single backslash before letter)
    $inText = [regex]::Replace($inText, '\\frac\{([^{}]+)\}\{([^{}]+)\}', '($1)/($2)')
    $inText = [regex]::Replace($inText, '\\sqrt\{([^{}]+)\}', 'sqrt($1)')
    $inText = [regex]::Replace($inText, '\\text\{([^{}]*)\}', '$1')

    $DPi      = [string][char]0x03C0
    $DTheta   = [string][char]0x03B8
    $DDelt    = [string][char]0x0394
    $DSigma   = [string][char]0x03A3
    $DAlpha   = [string][char]0x03B1
    $DBeta    = [string][char]0x03B2
    $DGamma   = [string][char]0x03B3
    $DMu      = [string][char]0x03BC
    $DRho     = [string][char]0x03C1
    $DLambda  = [string][char]0x03BB
    $DOmega   = [string][char]0x03C9
    $Dtimes   = [string][char]0x00D7
    $Dapprox  = [string][char]0x2248
    $Dleq     = [string][char]0x2264
    $Dgeq     = [string][char]0x2265
    $Ddeg     = [string][char]0x00B0
    $Ddot     = [string][char]0x00B7
    $Ddiv     = [string][char]0x00F7
    $Dpm      = [string][char]0x00B1
    $DSup2    = [string][char]0x00B2
    $DSup3    = [string][char]0x00B3
    $DSup4    = [string][char]0x2074
    $DSqrt    = [string][char]0x221A

    $pairs = @{
        "\pi"     = $DPi
        "\theta"  = $DTheta
        "\Delta"  = $DDelt
        "\Sigma"  = $DSigma
        "\alpha"  = $DAlpha
        "\beta"   = $DBeta
        "\gamma"  = $DGamma
        "\mu"     = $DMu
        "\rho"    = $DRho
        "\lambda" = $DLambda
        "\omega"  = $DOmega
        "\times"  = $Dtimes
        "\approx" = $Dapprox
        "\leq"    = $Dleq
        "\geq"    = $Dgeq
        "\deg"    = $Ddeg
        "\circ"   = $Ddeg
        "\cdot"   = $Ddot
        "\div"    = $Ddiv
        "\pm"     = $Dpm
        "\sin"    = "sin"
        "\cos"    = "cos"
        "\tan"    = "tan"
        "\sqrt"   = $DSqrt
        "\mathrm" = ""
        "\text"   = ""
        "\left"   = ""
        "\right"  = ""
    }
    foreach ($k in $pairs.Keys) {
        $inText = $inText.Replace($k, $pairs[$k])
    }

    $inText = $inText.Replace('{', '').Replace('}', '')
    $inText = $inText.Replace('^2', $DSup2).Replace('^3', $DSup3).Replace('^4', $DSup4)
    $inText = [regex]::Replace($inText, '\^\d+', '')
    $inText = $inText.Replace('$', '').Replace('`', '')

    # Catch-all: convert \frac to slash, strip ALL remaining backslashes
    $inText = $inText.Replace('\frac', '/')
    $inText = $inText -replace '\\', ''

    $inText = $inText -replace '\s+', ' '
    return $inText.Trim()
}

# --- Read and parse the NEW source file ---
$raw = [System.IO.File]::ReadAllText($sourceFile, [System.Text.UTF8Encoding]::new($false))
$raw = $raw -replace "`r`n", ' ' -replace "`n", ' ' -replace "`r", ' '
# Rule 1: Normalize runs of 2+ backslashes to exactly two
$raw = [regex]::Replace($raw, '\\{2,}(?![\\"/])', '\\\\')
# Rule 2: Escape lone backslashes that are not valid JSON escapes
$raw = [regex]::Replace($raw, '(?<!\\)\\(?![\\"/bfnrtu])', '\\\\')

try {
    $newData = $raw | ConvertFrom-Json
} catch {
    Write-Output "PARSE FAILED for $sourceFile"
    throw $_
}

# --- Load EXISTING geotechnical.json ---
$existingRaw = [System.IO.File]::ReadAllText($targetFile, [System.Text.UTF8Encoding]::new($true))
try {
    $existing = $existingRaw | ConvertFrom-Json
} catch {
    Write-Output "PARSE FAILED for existing $targetFile"
    throw $_
}

# Build lookup of existing IDs AND normalized question texts (content dedup)
$existingIds = @{}
$existingTexts = @{}
foreach ($q in $existing) {
    if ($q.id) { $existingIds["$($q.id)"] = $true }
    if ($q.question) {
        $norm = ($q.question -replace '\s+', ' ').Trim().ToLower()
        $existingTexts[$norm] = $true
    }
}

$newCount = 0
$skipped = @()
foreach ($q in $newData) {
    $answer = if ($q.correct_answer) { $q.correct_answer } elseif ($q.answer) { $q.answer } else { $null }
    if (-not $answer) {
        $skipped += "$($q.question_number) : $($q.question)"
        continue
    }
    # Content-based duplicate check (IDs may collide with unrelated old entries)
    $cleanQ = Clean-Text $q.question
    $normNew = $cleanQ.ToLower()
    if ($existingTexts.ContainsKey($normNew)) {
        Write-Output "SKIP content duplicate: $($q.question_number)"
        continue
    }
    $existingTexts[$normNew] = $true

    # Prefix IDs with 'sm_' so they never collide with old numeric ids
    $qid = "sm_$($q.question_number)"
    if ($existingIds.ContainsKey($qid)) {
        Write-Output "SKIP duplicate id: $qid"
        continue
    }
    $existingIds[$qid] = $true

    # [ordered] preserves insertion order; Sort-Object guarantees A,B,C,D...
    $cleanOpts = [ordered]@{}
    if ($q.options) {
        foreach ($prop in ($q.options.PSObject.Properties | Sort-Object Name)) {
            $cleanOpts[$prop.Name] = Clean-Text ([string]$prop.Value)
        }
    }
    $existing += [ordered]@{
        id          = $qid
        topic       = "Geotechnical Engineering"
        question    = $cleanQ
        options     = $cleanOpts
        answer      = $answer.ToString().ToLower()
        explanation = if ($q.explanation) { Clean-Text $q.explanation } else { "" }
    }
    $newCount++
}

$json = $existing | ConvertTo-Json -Depth 8
[System.IO.File]::WriteAllText($targetFile, $json, [System.Text.UTF8Encoding]::new($true))
Write-Output "Added $newCount new geotechnical questions. Total now: $($existing.Count)"
if ($skipped.Count -gt 0) {
    Write-Output "Skipped (no answer): $($skipped -join '; ')"
}